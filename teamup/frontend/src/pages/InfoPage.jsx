...
function ContactPage({ page }) {
  ...
  function submit(event) {
    ...
    window.location.href = `mailto:laurentcabinet74@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
    setSent(true);
  }
...
function DocumentPage({ page }) {
  if (page.legal) return <><Hero page={page} /><section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6"><div className="grid gap-4 text-sm md:grid-cols-[160px_1fr]">{[['Éditeur du site', 'TeamUp SAS'], ['Forme juridique', 'Société par actions simplifiée au capital de 10 000 €'], ['Siège social', '123 avenue de la Défense, 92000 Nanterre, France'], ['Immatriculation', 'RCS Nanterre 123 456 789'], ['Directeur de la publication', 'TeamUp'], ['Contact', 'support@teamup.app']].map(([key, value]) => <div key={key} className="contents"><strong className="border-b border-slate-100 py-3 text-slate-700">{key}</strong><span className="border-b border-slate-100 py-3 text-slate-500">{value}</span></div>)}</div></section></>;
