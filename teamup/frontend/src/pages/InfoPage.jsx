...
function DocumentPage({ page }) {
  if (page.legal) return <><Hero page={page} /><section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6"><div className="grid gap-4 text-sm md:grid-cols-[160px_1fr]">{[['Éditeur du site', 'Mehdi Benabid'], ['Nature du projet', 'Projet réalisé dans le cadre de la formation au Titre Professionnel Développeur Web et Web Mobile (DWWM)'], ['Nom du projet', 'TeamUp'], ['Directeur de la publication', 'Mehdi Benabid'], ['Contact', 'Utilise le formulaire de contact du site']].map(([key, value]) => <div key={key} className="contents"><strong className="border-b border-slate-100 py-3 text-slate-700">{key}</strong><span className="border-b border-slate-100 py-3 text-slate-500">{value}</span></div>)}</div></section></>;
