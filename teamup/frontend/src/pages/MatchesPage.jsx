...
export default function MatchesPage() {
  ...
  function handleSearchClick() {
    ...
    setServerStatus(`${count} ${count > 1 ? 'matchs trouvés' : 'match trouvé'} via recherche serveur.`);
  }
    ...
    return (
      <div className="mx-auto max-w-[1180px]">
        <PagePanel className="p-7">
          ...
          <div className="mt-6">
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-14 text-center">
              <p className="text-lg font-black text-slate-900">Chargement des matchs…</p>
              <p className="mt-2 text-sm text-slate-500">Récupération des parties disponibles près de chez toi.</p>
            </div>
          </div>
        </PagePanel>
      </div>
    ...
    <div className="mx-auto max-w-[1180px]">
      <PagePanel className="p-7">
        <div className="flex flex-wrap items-end justify-between gap-5">
          <PageTitle title="Trouver un match" subtitle="Trouve la partie qui te corresponds." />
        ...
        <div className="rounded-xl bg-white">
          <label className="flex items-center gap-3 rounded-lg border border-slate-200 px-4 py-3">
            ...
            <input
              value={query}
              onChange={(event) => setSearchParams({ ...Object.fromEntries(searchParams), search: event.target.value })}
              className="min-w-0 flex-1 text-sm font-medium outline-none"
              placeholder="Rechercher un match, une ville..."
            />
          </label>
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-[1fr_1fr_1fr_1fr_auto]">
            <SelectInput
              value={sport}
              onChange={(event) => setSearchParams({ ...Object.fromEntries(searchParams), sport: event.target.value })}
            >
            ...
            <div className="relative">
              <TextInput
                value={city}
                onChange={(event) => setSearchParams({ ...Object.fromEntries(searchParams), city: event.target.value })}
                list="teamup-cities"
                placeholder="Ville"
                aria-label="Ville"
                className="w-full"
            ...
            <TextInput type="date" aria-label="Date" value={date} onChange={(event) => setSearchParams({ ...Object.fromEntries(searchParams), date: event.target.value })} />
            <SelectInput
              value={level}
              onChange={(event) => setSearchParams({ ...Object.fromEntries(searchParams), level: event.target.value })}
            >
          ...
          {viewMode === 'map' ? (
            filteredMatches.length ? <MatchMap matches={filteredMatches} /> : (
              <div className="grid h-[600px] place-items-center rounded-xl border border-dashed border-slate-300 bg-slate-50 text-center">
                <div>
                  <p className="text-lg font-black text-slate-900">Aucun match à afficher sur la carte</p>
                  <p className="mt-2 text-sm text-slate-500">Modifie les filtres pour voir les matchs.</p>
                </div>
              </div>
            )
